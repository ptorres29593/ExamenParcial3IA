...
def elitist_selection(population, distances, num_elites):
    fitness_scores = [fitness(journey, distances) for journey in population]
    sorted_indices = np.argsort(fitness_scores)
    elites = [(population[i], fitness_scores[i]) for i in sorted_indices[:num_elites]]
    return elites
...
if __name__ == '__main__':
    ...
    initial_population = evolutionary_algorithm(population_size, num_cities)
    elites = elitist_selection(initial_population, gen_distances(), num_elites)

    for elite in elites:
        journey, distance = elite
        print("Ruta:", " -> ".join(journey) + ".")
        print("Distancia total:", distance, "km")
        print()